...
export default function LoginScreen({ navigation, route }) {
  ...
  // strip +91 if coming from registration
const [phone, setPhone] = useState(
  route?.params?.phone?.replace("+91", "") || ""
);
  ...
  const sendOtp = async () => {
    ...
    try {
       const fullPhone = "+91" + phone; // Add +91 here only
      const res = await fetch("http://10.35.57.52:5000/api/send-otp", {
        method: "POST",
      ...
      if (data.success) {
        ...
        Alert.alert("Failed to send OTP", data.message || "Try again");
      }
    } catch (err) {
      console.log(err);
      Alert.alert("Network error", "Could not send OTP. Check server connection.");
    }
  };
      ...
      >
        <View style={styles.container}>
          ...
          <View style={styles.card}>
            ...
            <TouchableOpacity style={styles.btn} onPress={mode === "otp" ? sendOtp : () => alert("Login Successful")}>
              <LinearGradient colors={["#ff0033", "#ff6600"]} style={styles.gradientBtn}>
                <Text style={styles.btnText}>{mode === "otp" ? "Send OTP" : "Login"}</Text>
              </LinearGradient>
            </TouchableOpacity>
